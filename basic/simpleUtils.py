...
class PrettyTable:
    def __init__(self):
        ...
        self.colTitles: list[str] | None = None

        #############
        # Rendering options
        self.render_colVisWidths: list[int] | None = None
        self.render_dividers: str | None = "│"

    def appendRow(self, row: list[str]):
        self.rows.append(row)

    def appendCol(self, column_to_add: list[str]):
        maxRows = max(len(self.rows), len(column_to_add))
        numCols = max(len(row) for row in self.rows) if self.rows else 0
        ...
        for i in range(maxRows):
            ...
            while len(row) < numCols:
                row.append("")  # Append empty strings to match the number of columns
            row.append(column_to_add[i] if i < len(column_to_add) else "")

    def getMaxColVisWidths(self) -> list[int]:

        visWidths_calc: list[int] = []

        def row_review(row: list[str] | None):
            if row:
                for i, col in enumerate(row):
                    wid = PrettyText.uniLen_approx(col)
                    if len(visWidths_calc) <= i:
                        visWidths_calc.append(wid)
                    elif visWidths_calc[i] is None or (wid > visWidths_calc[i]):
                        visWidths_calc[i] = wid

        for row in self.rows:
            row_review(row)

        #####################################################
        # Titles: Only if column isn't empty
        if self.colTitles:
            for i, title in enumerate(self.colTitles or []):
                if i < len(visWidths_calc):
                    if visWidths_calc[i] > 0:
                        wid = PrettyText.uniLen_approx(title)
                        if wid > visWidths_calc[i]:
                            visWidths_calc[i] = wid

        return visWidths_calc

    def asLines(
        self,
        render_divider: str = " ",  # "│",
        render_colVisWidths: list[int] | None = None,
    ) -> list[str]:

        visWidths_calc = self.getMaxColVisWidths()

        visWidths_used = []

        for i in range(len(visWidths_calc)):
            if (
                render_colVisWidths
                and (i < len(render_colVisWidths))
                and (render_colVisWidths[i] is not None)
            ):
                visWidths_used.append(render_colVisWidths[i])
            else:
                visWidths_used.append(visWidths_calc[i])

        def row_asText(row: list[str] | None) -> str:
            txtOut = ""
            if row:
                for i, col in enumerate(row):
                    if i >= len(visWidths_used):
                        break

                    wid = visWidths_used[i]
                    if wid > 0:
                        if render_divider is not None and (txtOut != ""):
                            txtOut += render_divider
                        visLen = PrettyText.uniLen_approx(col)
                        txtOut += f"{col}{' '*(wid-visLen)}"
            return txtOut

        lines: list[str] = []
        if self.colTitles:
            lines.append(row_asText(self.colTitles))
    ...
    def tableDump(
        ...
        for line in self.asLines(render_divider, render_colVisWidths):
            print(line)
