...
class CustomisedContents:
    ...
    def _parseAttributesFromRawData(self, rawData: bytes) -> bool:
        ...
        if isinstance(values, str):
            appLog.print_warning(
                f"Loading header from raw data[{rawData[:32].hex()}]…->⚠️  {asJsonStr(values,indent=2)}"
            )
    ...
    def __str__(self):
        if not self.hasErrors():
            return (
                f"{self.name}({self.headerFormatText})=\n{asJsonStr(self.attributes)}"
            )
        else:
            return f"{self.name}({self.headerFormatText})=⚠️ {self.errorMsgs(' | ')} ⚠️ {asJsonStr(self.attributes)}"
...
class DataHeaderFormat:
    ...
    def loadHeaderFromBytes(self, rawData: bytes) -> dict[str, Any] | str:
        ...
        #
        try:
            appLog.print_verbose(f"Final: {asJsonStr(theHeaderValues,indent=2)}")
        except Exception as e:
        ...
        for example_kind, example_files in jsonData["examples"].items():
            for example_file in example_files:
                if not os.path.isfile(example_file):
                    appLog.print_warning(
                        f"CustomFormat[{pathAsDisplay(fname)}].examples[{example_kind}]: Missing file: {pathAsDisplay(example_file)}"
                    )
