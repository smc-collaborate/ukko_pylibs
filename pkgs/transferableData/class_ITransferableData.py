...
from imageProcessing import ImageInfo
...
class ITransferableData:
    ...
    def create_error(
        ...
        from appAssist import appGetValue

        if appGetValue("includeErrorCodesInResponses", True) and (errCode != ""):
        ...
        sys.stderr.write(
            f"⚠️ Creating error{prefix}: {ukkoUtils.asJsonStr(json_dict)}\n"
        )
        ...
        while isinstance(errDetails_dict, dict):
            ...
            if (err_msg is None) and (err_code is None):
                ...
                if len(errDetails_dict) > 0:
                    errTxt += f" | Details: {ukkoUtils.asJsonStr(errDetails_dict)}"
                return errTxt
        return None
    ...
    def appendWarnings(self, warningsList: list | None) -> None:
        if warningsList is not None and len(warningsList) > 0:
            dictUtils.extendWithoutOverwrite(
                self.dict_annotations, {"warnings": warningsList}
            )

    def appendErrors(self, errorsList: list | None) -> None:
        if errorsList is not None and len(errorsList) > 0:
            dictUtils.extendWithoutOverwrite(
                self.dict_annotations, {"errors": errorsList}
            )

    def appendAnnotations(self, extraValues: dict[str, Any] | None) -> None:
        dictUtils.extendWithoutOverwrite(self.dict_annotations, extraValues)
    ...
    def getJsonWithExtras(self) -> dict[str, Any]:
        ...
        if timestamp_or_zero is not None and timestamp_or_zero > 0:
            _noteValue(
                json_dict,
                "timestamp",
                ukkoUtils.timestampObj_from_ns(timestamp_or_zero),
                prev,
            )
