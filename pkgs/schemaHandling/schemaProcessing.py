...
from ukkoUtils import asJsonStr, pathAsDisplay
...
class enum_ItemExpectation(Enum):
    ...
    def fromExpectationValueOrNone(
        ...
        appLog.print_warning(
            f"enum_ItemExpectation: `{asJsonStr(value)}` - Defaulting to {defaultValue}"
        )
...
class SchemaUsageInstructions:
            ...
            try:
                ...
                for key, value in srcObj.items():
                    self.usageRulesList.append(
                        f"Usage Rules[{key}]: `{asJsonStr(value)}`"
                    )

            except Exception as e:
                appLog.print_warning_withException(
                    e,
                    f"Error processing SchemaUsageInstructions: {asJsonStr(srcObj)}",
                )
    ...
    def getBitstreamExpectation(self, jsonData: dict[str, Any] | None) -> bool | None:
        ...
        if self.bitstreamExpectation == enum_ItemExpectation.REQUIRED:
            ...
            appLog.print_warning(
                f"SchemaUsageRules.getBitstreamExpectation: Invalid value {asJsonStr(self.bitstreamExpectation)}"
            )
...
class Schema:
    ...
    def __init__(
        ...
        try:
            ...
            self.errMsg = f"Failed to load schema from {pathAsDisplay(self.jsonSchemaFilename)}: {str(e)}"
            appLog.print_warning(
    ...
    def doValidateJson(self, inputJson: dict[str, Any] | None) -> str | None:
        ...
        try:
            ...
            warn_msg = f"JSON schema validation error: {'/'.join([str(item) for item in e.relative_path])}={asJsonStr(e.instance)} | {e.message}"
        ...
        if warn_msg is not None and (warn_msg != ""):
            ...
            appLog.print_warning(
                f"---------------------------\ndoValidateJson({self.name}): {asJsonStr(failure_obj, indent=2)}\n---------------------------"
            )
    ...
    try:
        for key in srcObj:
            ...
            if value == "[Value]":
                ...
                valueTxt = f"`{asJsonStr(srcObj[key])}`"
            else:
                valueTxt = str(value)
...
# @todo: Replace with markdown.MarkdownTable
...
class SchemaDocMarkdown:
    ...
    def fullSchemaToTable(
        ...
        for key, value in srcObj.items():
            table.addExtraNote(
                f"FullSchemaExtra: {key}=`{asJsonStr(value)}`",
                allowDuplicates=False,
            )
    ...
    def jsonSchemaAddEntry(
        ...
        def _getLimits(minKey: str, maxKey: str, name: str) -> str:
            ...
            if example is not None:
                exampleStr = "eg: " + asJsonStr(example)
            ...
            if const is not None:
                constStr = "Always " + asJsonStr(const)
        ...
        try:
            ...
            if out_type == "string":
                ...
                for key in srcObj:
                    out_notes += f" {key}={asJsonStr(srcObj[key])}"
    ...
    def dumpRemaining(self, srcObj_: dict[str, Any] | None, prefix: str = "") -> str:
        ...
        if srcObj_ != None:
            for key, value in srcObj_.items():
                ...
                entry_md += "```json\n"
                entry_md += asJsonStr(value, indent=2)
                entry_md += "\n```\n"
    ...
    def usageRulesSchemaToMarkdown(self, srcObj_: dict[str, Any] | None) -> list[str]:
        ...
        for key, value in srcObj.items():
            lines.append(f"Usage Rules Descriptor[{key}]: `{asJsonStr(value)}`")
    ...
    def itemInfoToFriendlyStr(obj_in: dict[str, Any] | None):
        ...
        for name, value in obj.items():
            extras.append(f"{name}={asJsonStr(value)}")
        if len(extras) > 0:
            result += " [" + ", ".join(extras) + "]"
        return result
