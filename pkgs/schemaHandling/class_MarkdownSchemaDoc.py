...
from schemaHandling import SchemaUsageInstructions
...
class SchemaEntry:
    ...
    def getLimits(
        ...
        if example is not None:
            exampleStr = "eg: " + ukkoUtils.asJsonStr(example)
        ...
        try:
            ...
            if dataToAdd.typeIs(["number", "integer"]):
                ...
                if (
                    ...
                    out_type = f"[{out_type} …]"
                    notes += [
                        ukkoUtils.rangeAsText(minItems, maxItems, "Array length: ")
                    ]
...
class MarkdownSchemaDoc:
    ...
    def fullSchemaToTable(
        ...
        * srcObj.[anything else] -> added as extra notes to the table"""
        ...
        for key, value in srcObj.items():
            dest.destTable.addExtraNote(
                f"FullSchemaExtra: {key}=`{ukkoUtils.asJsonStr(value)}`",
                allowDuplicates=False,
            )
    ...
    def dumpRemaining(self, srcObj_: dict[str, Any] | None, prefix: str = "") -> str:
        ...
        if srcObj_ != None:
            for key, value in srcObj_.items():
                ...
                entry_md += "```json\n"
                entry_md += ukkoUtils.asJsonStr(value, indent=2)
                entry_md += "\n```\n"
    ...
    def usageRulesSchemaToMarkdown(self, srcObj_: dict[str, Any] | None) -> list[str]:
        ...
        for key, value in srcObj.items():
            lines.append(
                f"Usage Rules Descriptor[{key}]: `{ukkoUtils.asJsonStr(value)}`"
            )
