# -*- coding: utf-8 -*-
# From: git@github.com:nevermoe/protobuf_decoder.git  --hash:0d5dfe812a3bf0f2ac2ff57fabf35f104328b77
import struct
from typing import Tuple


def decodeProtobuf_binToDict(binary):
    messages = {}
...
# return None -or-(num, newStart, success)
def RetrieveInt(data, start, end) -> None | Tuple[int, int]:
    pos = 0
    byteList = []
    while True:
        if start + pos >= end:
...
def ParseRepeatedField(data, start, end, message, depth=0):
    while start < end:
        _result = RetrieveInt(data, start, end)
        if _result == None:
            return False
        (num, start) = _result
        message.append(num)
...
def ParseData(data, start: int, end: int, messages: dict, depth: int = 0):
    global strings
    # print strings
    ordinary = 0
    while start < end:
        _results = GetDynamicWireFormat(data, start, end)

        if _results == None:
            return False

        (start, wire_type, field_number) = _results
        if wire_type == 0x00:  # Varint
            ...
            if _result == None:
                return False
            (stringLen, start) = _result
            # stringLen = (data[start+1])
            if depth != 0:
                strings.append("\t" * depth)
            ...
            if start + stringLen > end:
                ...
                messages.pop(
                    "%02d:%02d:embedded message" % (field_number, ordinary), None
                )
                return False
            ...
            if ret == False:
                ...
                _asString = False
                try:
                    if not (0 in data[start : start + stringLen]) and not (
                        0xFF in data[start : start + stringLen]
                        ...
                        data[start : start + stringLen].decode(
                            "utf-8"
                        )  # .encode('utf-8')
                        strings.append(
                            "(%d) string: %s\n"
                        ...
                        messages["%02d:%02d:string" % (field_number, ordinary)] = data[
                ...
                if _asString:
                    ...
                    if ret == False:
                        del strings[curStrIndex + 1 :]  # pop failed result
                        messages.pop(
                            "%02d:%02d:repeated" % (field_number, ordinary), None
                        )
...
def ReEncode(messages, output):
    ...
    for key in sorted(messages.iterkeys(), key=lambda x: int(x.split(":")[1])):
        ...
        if wire_type == "Varint":
            ...
            bytesStr = [
                int(elem.encode("hex"), 16) for elem in messages[key].encode("utf-8")
            ]
