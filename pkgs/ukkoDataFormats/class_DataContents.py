...
from ._externals.parseProtoBuf import decodeProtobuf_binToDict
...
class DataContents(DataSourceWithInfo):
    ...
    def asParamTxt(self) -> str:
        ...
        if self.asData is None:
            ...
            try:
                resultTxt = ukkoUtils.asJsonRStr(self.asData, sortKeys=True)
    ...
    def asTextLines(self) -> list[str]:
        ...
        if self.contentType.isValue("protobuf"):
            try:
                ...
                return ukkoUtils.asJsonStr(decoded_message, indent=2).splitlines()
            except Exception as e:
    ...
    def getAsDisplay(self, clipLen: int = CLIP_LENGTH) -> str:
        ...
        if self.asProvidedAfterPrefix != "" and isinstance(
            ...
            if _fname:
                _paramText = self.prefix + ukkoUtils.pathAsDisplay(_fname)
    ...
    def asBytes(self) -> bytes:
        if isinstance(self.asData, bytes):
            ...
            return ukkoUtils.asJsonStr(self.asData).encode("utf-8")
        else:
            raise ValueError(
    ...
    def _doLoadExtendedData(
        ...
        try:
            ...
            if (
                ...
                _txtToReview = ukkoUtils.asUtf8orBytes(self.asData)
        ...
        if (
            ...
            if appLog.isVerbose():
                ...
                self.print_verbose(
                    ...
                    + ukkoUtils.asJsonRStr(msgOld)
                    + " → "
                    + ukkoUtils.asJsonRStr(msgNew)
                )
