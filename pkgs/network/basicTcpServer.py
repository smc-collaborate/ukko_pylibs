...
class BasicTcpServer(Interface_BasicServerOut):
    ...
    def sendToNewConnections(self, dataIn: bytes | str) -> int:
        ...
        if sentCount > 0:
            appLog.print_info(
                f"Sent to {pluralize(sentCount, 'connection')}: {pluralize(numBytesSentPerConnection, 'byte')}"
            )
    ...
    def doBroadcast(self, dataIn: bytes | str | None) -> int:
        ...
        if sentCount == 0:
            ...
            appLog.print_verbose(
                f"Broadcasted to {pluralize(sentCount, 'connection')}: {pluralize(numBytesSentPerConnection, 'byte')}"
            )
