...
class ValueHelpSummary:
    ...
    @staticmethod
    def asTableRowRenderOptions() -> PrettyData.TableRenderOptions.Columns:
        return PrettyData.TableRenderOptions.Columns.create_fromList(
            [
                None,
                PrettyData.ColumnSpec(50),
                PrettyData.ColumnSpec(72),
                None,
                PrettyData.ColumnSpec(102, [" • "]),
            ]
        )

    def asTableRow(self) -> PrettyData.Row:
        return PrettyData.Row.create_fromList(
            [
                self.shortName,  # M 0
...
class ValueHelpSummaries:
    ...
    def asTable(self) -> PrettyData.Contents:
        ...
        for group in groupList:
            ...
            groupTable = PrettyData.Contents(
                [
                    "",
    ...
    def asTextLines(self) -> list[str]:
        table = self.asTable()
        rendered = PrettyData.Rendered(
            table,
            PrettyData.TableRenderOptions(
                {"colOptions": ValueHelpSummary.asTableRowRenderOptions()}
            ),
        )
