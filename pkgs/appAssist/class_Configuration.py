...
from appLogging import SimpleLogger
        ...
        if config_fname is not None:
            ...
            try:
                with open(config_fname, "r", encoding="utf-8") as f:
                    ...
                    self.notes.append(
                        f"Loaded config file '{ukkoUtils.pathAsDisplay(config_fname)}'"
                    )
            except Exception as e:
                self.log_error(f"Unable to load config file '{config_fname}'", e)
                self.notes.append(
                    f"Unable to load from config file '{ukkoUtils.pathAsDisplay(config_fname)}'"
                )
