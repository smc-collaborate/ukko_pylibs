...
# +--------------------------------------------------------------------------------------------------------
# | from ukkoAppTemplates import jsonLineStreamingApp_doRun, IJsonLineStreamerSpec
# |
# | def main():
...
class JsonLineStreamingApp:
    ...
    def doCollectDataAndStream(
        ...
        while self.dataStreamer.hasConnections():
            ...
            appLog.print_verbose(f"Reviewed data: {asJsonStr(data,indent=2)}")
            sentCount = self.dataStreamer.doBroadcast(asJsonStr(data))
