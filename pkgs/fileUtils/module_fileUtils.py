...
def loadJsonDictFromFile(
    ...
    fname_friendly = pathAsDisplay(inputJsonFile)
    errmsg = "Unknown Error"
    showWarning = True

    from appAssist import app  # < Not permitted to be imported at module-level
...
def fileExceptionToString(fname: str, e: Exception, what: str = "file") -> str:
    msg = f"Unable to load {what} from file '{pathAsDisplay(fname)}'\n"
    ...
    if fullPath != pathAsDisplay(fname):
        msg += f"\nFull path  : {fullPath}"
...
def loadJson(
    ...
    try:
        ...
        if exceptionOnError:
            ...
            errmsg = f"Unable to load {inputKind} from '{pathAsDisplay(inputJson)}': A [{type(e).__name__}] exception occurred: {e}"
    except json.JSONDecodeError:
        errmsg = (
...
def loadJson_dict_withSourceDescription_orException(
    ...
    if (inputJson == None) or (inputJson == "") or (inputJson == "null"):
        ...
        sourceDescription = f"{pathAsDisplay(inputJson[1:])}"
...
def exportToFile_orHandledException(
    ...
    if isinstance(fileContents_, dict):
        fileContentsOut = asJsonStr(fileContents_, indent=2, sortKeys=True) + "\n"
    else:
    ...
    try:
        ...
        if outputFilename == "/dev/stderr":
            ...
            if outputFilename == "/dev/stdout":
                if isStdoutText() and not (isText):
